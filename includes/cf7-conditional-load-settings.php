...
$parent = null;
	...
	function cf7cl_add_page() {
		$page_title = __( 'Conditionally Load CF7 Settings', 'cf7-conditional' );
		$menu_title = __( 'Conditionally Load CF7', 'cf7-conditional' );
...
?>
<div class="wrap">
	<h2><?php _e( 'Conditionally Load CF7', 'cf7-conditional' ) ?></h2>
...
<?php
	...
	<ul>
		...
		<li>' . __( 'Your account isn’t an Administrator or Editor account' , 'cf7-conditional-load' ) . '</li>
		<li>' . __( 'An admistrator changed your account level' , 'cf7-conditional-load' ) . '</li>
	</ul>
	<p>' . __( 'Please log in as an Administrator or Editor or contact the site adminstrator to resolve this issue.' , 'cf7-conditional-load' ) . '</p>
</div>';
		}
	}
	...
	// Create Setting
		...
		$title            = __( 'Load Contact Form 7 scripts only on pages that need them', 'cf7-conditional' ); // Section heading (<h2>).
	...
	function cf7cl_text() {
		echo '<p>'. __( 'In its default settings, Contact Form 7 loads its JavaScript and CSS stylesheet on every page. This slows page loading and taxes server and client resources. In worst case scenarios, slow page loading can ding your <abbr>SEO</abbr> or drive away potential users. This setting gives you control over which pages the scripts load on.', 'cf7-conditional' ) . '</p><figure><img style="max-width:100%;height:auto;border:5px solid gray;border-radius:5px;" src="'. CF7CL_URL .'images/page-id.png" alt="' . esc_html__( 'How to find a page or post ID', 'cf7-conditional' ) . '" width="1024" height="488" class="size-full" srcset="'. CF7CL_URL .'images/page-id.png 1024w, '. CF7CL_URL .'images/page-id-300x143.png 300w, '. CF7CL_URL .'images/page-id-768x366.png 768w, '. CF7CL_URL .'images/page-id-1024x488.png 1024w, '. CF7CL_URL .'images/page-id-586x279.png 586w" sizes="(max-width: 1188px) 100vw, 1188px"><figcaption></figcaption>' . __( 'This is the page (or post) <abbr>ID</abbr>', 'cf7-conditional' ) . '</figure><figure><img style="max-width:100%;height:auto;border:5px solid gray;border-radius:5px;" src="'. CF7CL_URL .'images/page-slug.png" alt="' . esc_html__( 'How to find a page or post slug', 'cf7-conditional' ) . '" width="1024" height="202" class="size-full" srcset="'. CF7CL_URL .'images/page-slug.png 1024w, '. CF7CL_URL .'images/page-slug-300x59.png 300w, '. CF7CL_URL .'images/page-slug-768x152.png 768w, '. CF7CL_URL .'images/page-slug-1024x202.png 1024w, '. CF7CL_URL .'images/page-slug-586x116.png 586w" sizes="(max-width: 1188px) 100vw, 1188px"><figcaption>' . __( 'This is the page (or post) slug.', 'cf7-conditional-load') .'</figcaption></figure><p>' . __( 'Enter a list of the page <abbr>ID</abbr>s (numerical) or page slugs (lowercase letters, numerals, &amp; hyphens) where you want to load the stylesheet and javascript for Contact Form 7. One page <abbr>ID</abbr> or slug per line.', 'cf7-conditional' ) . '</p>';
	}
	...
	function cf7cl_option_render() {
		...
		echo '<label for="cf7cl_conditional_load" style="display:block;margin-bottom:15px"><abbr>ID</abbr>s and/or slugs</label><textarea rows="5" cols="50" placeholder="' . __( 'Enter one page <abbr>ID</abbr> or slug per line.', 'cf7-conditional' ) . '" name="cf7cl_conditional_load" id="cf7cl_conditional_load" value="' . $result .'">' . $result . '</textarea>';
	}
...
<div class="error notice is-dismissible">
	<p><?php _e( 'You have installed and activated the Contact Form 7 plugin. <a href="' .admin_url( 'options-general.php?page=cf7-conditional-load' ). '">Please configure Conditionally Load CF7 Settings</a>.', 'cf7-conditional' ); ?></p>
</div>
<?php
		}
	}
